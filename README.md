...
- **Persona detail** - modal with description, stats, affinities, learned
  skills (with element icons), fusion recipes, and, where relevant, its Theurgy
  fusion-spell pair. Deep-linkable at `/persona/<query>`.
- **Fusion** - forward result in Compare (two personas -> product, click to
  open it), a 22x22 arcana fusion matrix, a special-recipe reference and the
  seven protagonist Theurgy fusion spells, all on `/arcana`.
...
- **Skills** - `/skills` catalog with owners and a "how skills work" guide.
